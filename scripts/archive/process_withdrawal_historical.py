...
def get_nav_for_date(conn, transaction_date):
    ...
    # Try exact date - TABLE NAME: daily_nav
    cursor.execute("""
...
def process_withdrawal(conn, investor_id, investor_name, investor_email,
    ...
    try:
        ...
        # Record tax event - COLUMN NAME: date (not event_date)
        cursor.execute("""
            INSERT INTO tax_events (
                investor_id, date, withdrawal_amount,
                realized_gain, tax_due, net_proceeds, tax_rate, notes
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            investor_id,
            transaction_date,
...
def main():
    ...
    try:
        ...
        print(f"   Or enter date: YYYY-MM-DD")
        print()
