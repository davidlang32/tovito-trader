...
- Shows current value based on today's NAV
- Displays gain/loss since contribution
"""
...
try:
    ...
    print("⚠️  Warning: python-dotenv not installed, using environment variables")
...
try:
    ...
    print("⚠️  Warning: Email service not available")
...
def get_nav_for_date(conn, transaction_date):
    ...
    if result:
        print(f"⚠️  No NAV found for {transaction_date}, using most recent: {result[0]}")
        return result
...
def process_contribution(conn, investor_id, investor_name, investor_email,
    ...
    try:
        ...
        if current_nav_data:
            ...
            today_nav = nav_per_share  # Fallback
        ...
        # Update NAV history with new totals
        cursor.execute("""
