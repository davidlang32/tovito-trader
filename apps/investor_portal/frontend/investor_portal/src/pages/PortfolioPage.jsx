...
const OverviewCard = ({ label, value, subtitle, icon: Icon, colorClass }) => (
  <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 relative overflow-hidden">
    <div className={`absolute left-0 top-0 bottom-0 w-1 ${colorClass || 'bg-gray-200'}`} />
...
const renderActiveShape = (props) => {
  ...
  return (
    <g>
      <text x={cx} y={cy - 12} textAnchor="middle" fill="#111827" fontSize={18} fontWeight={700}>
        {payload.name}
      </text>
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition ${
                activeIndex === i ? 'bg-gray-50 shadow-sm' : 'hover:bg-gray-50'
              }`}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-gray-900 ml-2">{formatCurrencyCompact(item.value)}</span>
                </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        ...
        <table className="w-full min-w-[700px]">
            ...
            <tr className="border-b border-gray-100">
              {SORT_FIELDS.map(f => (
                <th
                  ...
                  className={`text-left py-2.5 px-3 text-xs font-semibold uppercase tracking-wider cursor-pointer hover:bg-gray-50 transition select-none ${
                    f.key === 'symbol' ? '' : 'text-right'
                  } ${sortBy === f.key ? 'text-gray-700' : 'text-gray-400'}`}
                >
          ...
          <tbody>
            {sortedHoldings.length === 0 ? (
              <tr>
                <td colSpan={SORT_FIELDS.length} className="text-center py-8 text-gray-400 text-sm">
                ...
                >
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <div>
                        <span className="font-semibold text-gray-900 text-sm">{h.symbol}</span>
                        <p className="text-[10px] text-gray-400">{h.instrument_type}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-3 text-right">
                    <span className="font-semibold text-gray-900 text-sm">{formatCurrency(h.market_value)}</span>
                  </td>
                  ...
                  <td className="py-3 px-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <span className="text-sm text-gray-600 w-12 text-right">{h.weight_pct.toFixed(1)}%</span>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-3 text-right">
                    <span className={`font-semibold text-sm ${
                      (h.unrealized_pl || 0) >= 0 ? 'text-emerald-600' : 'text-red-600'
                    }`}>
                  ...
                  <td className="py-3 px-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span className={`text-sm font-medium ${
                        (h.unrealized_pl_pct || 0) >= 0 ? 'text-emerald-600' : 'text-red-600'
                      }`}>
                  ...
                  {/* Cost Basis */}
                  <td className="py-3 px-3 text-right">
                    <span className="text-sm text-gray-600">{h.cost_basis != null ? formatCurrency(h.cost_basis) : '--'}</span>
                  </td>
      ...
      {holdings && sortedHoldings.length > 0 && (
        <div className="flex flex-wrap gap-6 mt-4 pt-3 border-t border-gray-100 text-xs text-gray-500">
          <div>
            <span>Total Value: </span>
            <span className="font-bold text-gray-700">{formatCurrency(holdings.total_market_value)}</span>
          </div>
          <div>
            <span>Total Unrealized: </span>
            <span className={`font-bold ${holdings.total_unrealized_pl >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
...
const getDiversificationGrade = (hhi, holdings) => {
  ...
  if (hhi <= 1500) return { grade: 'A', label: 'Well Diversified', color: 'text-emerald-600', bg: 'bg-emerald-50', bar: '#10b981' };
  if (hhi <= 2500) return { grade: 'B', label: 'Moderately Diversified', color: 'text-blue-600', bg: 'bg-blue-50', bar: '#3b82f6' };
  if (hhi <= 4000) return { grade: 'C', label: 'Somewhat Concentrated', color: 'text-amber-600', bg: 'bg-amber-50', bar: '#f59e0b' };
  if (hhi <= 6000) return { grade: 'D', label: 'Highly Concentrated', color: 'text-orange-600', bg: 'bg-orange-50', bar: '#f97316' };
  return { grade: 'F', label: 'Very Concentrated', color: 'text-red-600', bg: 'bg-red-50', bar: '#ef4444' };
};
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        <SectionHeader
          ...
          iconBg="bg-amber-50"
        />
        <div className="text-center py-8 text-gray-400 text-sm">Not enough data for analysis</div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="p-3 rounded-xl bg-blue-50/50 border border-blue-100">
          <p className="text-[10px] text-blue-500 font-semibold uppercase tracking-wider mb-1">Largest Position</p>
          <p className="text-lg font-bold text-gray-900">{top1?.symbol || '--'}</p>
          <p className="text-xs text-gray-500">{top1?.weight_pct?.toFixed(1) || '--'}% of portfolio</p>
        </div>

        {/* Top 3 */}
        <div className="p-3 rounded-xl bg-purple-50/50 border border-purple-100">
          <p className="text-[10px] text-purple-500 font-semibold uppercase tracking-wider mb-1">Top 3 Holdings</p>
          <p className="text-lg font-bold text-gray-900">{top3Weight.toFixed(1)}%</p>
          <p className="text-xs text-gray-500">of total portfolio</p>
        </div>

        {/* Best Performer — historical */}
        <div className="p-3 rounded-xl bg-emerald-50/50 border border-emerald-100">
          <p className="text-[10px] text-emerald-500 font-semibold uppercase tracking-wider mb-1">Top Performer</p>
          {histPerformers?.top_performers?.length > 0 ? (
            <>
              <p className="text-lg font-bold text-emerald-700">{histPerformers.top_performers[0].symbol}</p>
            ...
            <>
              <p className="text-lg font-bold text-emerald-700">{bestPerformer?.symbol || '--'}</p>
              <p className="text-xs text-emerald-600">
        ...
        <div className="p-3 rounded-xl bg-red-50/50 border border-red-100">
          <p className="text-[10px] text-red-500 font-semibold uppercase tracking-wider mb-1">Needs Attention</p>
          {histPerformers?.bottom_performers?.length > 0 ? (
            <>
              <p className="text-lg font-bold text-red-700">{histPerformers.bottom_performers[0].symbol}</p>
            ...
            <>
              <p className="text-lg font-bold text-red-700">{worstPerformer?.symbol || '--'}</p>
              <p className="text-xs text-red-600">
      ...
      {histPerformers?.top_performers?.length > 1 && (
        <div className="mt-4 p-4 rounded-xl border border-gray-100 bg-gray-50/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {/* Top performers */}
            <div>
              <p className="text-[10px] text-emerald-500 font-semibold uppercase tracking-wider mb-1.5">Best Trades</p>
            ...
            {/* Bottom performers */}
            <div>
              <p className="text-[10px] text-red-500 font-semibold uppercase tracking-wider mb-1.5">Toughest Trades</p>
              <div className="space-y-1.5">
      ...
      {gradeInfo.isCash && (
        <div className="flex items-start gap-2 mt-4 p-4 bg-blue-50 rounded-xl border border-blue-200">
          <Info className="w-4 h-4 text-blue-500 flex-shrink-0 mt-0.5" />
          <div className="text-[11px] text-blue-700 leading-relaxed">
            <p className="font-semibold text-blue-800 mb-1">Why is the fund in cash or SGOV?</p>
            <p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20 lg:pb-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm">What the fund is invested in and how it's allocated</p>
      </div>
