...
- SMTP (default for local/desktop) — requires SMTP_SERVER, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD
- Resend HTTP API (for cloud hosting where SMTP is blocked) — requires RESEND_API_KEY

Set EMAIL_PROVIDER=resend to use Resend, or leave unset/smtp for SMTP.
...
class EmailService:
    ...
    Supports SMTP and Resend HTTP API transports.
    Transport is selected via EMAIL_PROVIDER env var:
        - 'smtp' (default): Traditional SMTP
        - 'resend': Resend HTTP API (for Railway/cloud where SMTP ports are blocked)

    Every send attempt is logged to the email_logs table for audit trail.
    """

    def __init__(self):
        self.provider = os.getenv('EMAIL_PROVIDER', 'smtp').lower().strip()
        self.email_from = os.getenv('SMTP_FROM_EMAIL', '')
        self.from_name = os.getenv('SMTP_FROM_NAME', 'Tovito Trader')

        if self.provider == 'resend':
            self.resend_api_key = os.getenv('RESEND_API_KEY', '')
            if not self.resend_api_key:
                print("[WARN] RESEND_API_KEY not configured - emails will not be sent")
            if not self.email_from:
                self.email_from = os.getenv('SMTP_USERNAME', '')
        else:
            # SMTP configuration
            ...
            if not self.email_from:
                self.email_from = self.smtp_user or ''

            if not self.smtp_user or not self.smtp_password:
                print("[WARN] SMTP credentials not configured - emails will not be sent")
    ...
    def _send_via_resend(self, to_email: str, subject: str, message: str,
                         html: bool = False, attachments: Optional[List[str]] = None) -> bool:
        """Send email via Resend HTTP API.

        Resend API docs: https://resend.com/docs/api-reference/emails/send-email
        Uses plain requests.post() — no SDK dependency needed.
        """
        import requests
        ...
        # Handle attachments (base64 encoded for Resend API)
        if attachments:
            import base64
            attachment_list = []
        ...
        if resp.status_code in (200, 201):
            result = resp.json()
            print(f"  [OK] Email sent via Resend (id: {result.get('id', 'unknown')})")
            return True
        else:
            error_detail = resp.text[:300]
            raise Exception(f"Resend API error {resp.status_code}: {error_detail}")

    def _send_via_smtp(self, to_email: str, subject: str, message: str,
                       html: bool = False, attachments: Optional[List[str]] = None) -> bool:
        """Send email via SMTP (traditional transport)."""
        if not self.smtp_user or not self.smtp_password:
    ...
    def send_email(self, to_email: str, subject: str, message: str,
        ...
        Automatically uses the transport configured via EMAIL_PROVIDER env var:
        - 'smtp' (default): SMTP transport
        ...
        try:
            if self.provider == 'resend':
                self._send_via_resend(to_email, subject, message, html, attachments)
            else:
                self._send_via_smtp(to_email, subject, message, html, attachments)
            ...
            try:
                ...
                print(f"      Email error: {ascii(str(e))}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("[WARN] Email not sent (check configuration)")
